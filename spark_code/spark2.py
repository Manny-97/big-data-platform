...
def create_spark_connection(
    env_type, aws_access_key, aws_secret_key, aws_region, jar_path
):
    ...
    try:
        ...
        if env_type == "local":
            ...
            conf.set("spark.hadoop.fs.s3a.access.key", aws_access_key).set(
                "spark.hadoop.fs.s3a.secret.key", aws_secret_key
            ).set("spark.hadoop.fs.s3a.endpoint", f"s3.{aws_region}.amazonaws.com").set(
                "spark.hadoop.fs.s3a.impl", "org.apache.hadoop.fs.s3a.S3AFileSystem"
            ).set(
                "spark.driver.extraClassPath",
                f"{jar_path}hadoop-aws-3.3.4.jar,{jar_path}aws-java-sdk-bundle-1.12.353.jar",
            ).set(
        ...
        conf.set("spark.sql.adaptive.enabled", "true").set(
            "spark.sql.adaptive.skewJoin.enabled", "true"
        ).set("spark.sql.files.maxPartitionBytes", str(200 * 1024 * 1024))

        # Define default builder
        builder = SparkSession.builder.appName("Processing_telcom_data").config(
            conf=conf
        )
...
def get_files_uploaded_last_24_hours(
    aws_access_key: str, aws_secret_key: str, bucket_name: str
):
    ...
    try:
        ...
        start_time = datetime.now(timezone.utc) - timedelta(
            days=1
        )  # Example: list out files uploaded every last 24 hours
...
def read_data_from_s3(spark, source_files: list):
    ...
    try:
        ...
        count = df.count()
        logger.info(
            f"Successfully read {count} records from {len(source_files)} files in {time.time() - start_time:.2f} seconds."
        )
        return df
...
def transform_data(spark, df):
    ...
    try:
        if df is None:
            ...
            df = (
                df.withColumn(
                    "call_duration_seconds", col("call_duration_seconds").cast("int")
                )
                .withColumn("message_length", col("message_length").cast("int"))
                .withColumn(
                    "signal_strength_dbm", col("signal_strength_dbm").cast("int")
                )
            ...
            df = (
                df.filter(col("sim_card_number").rlike("^8930\\d{15}$"))
            ...
            df = (
                df.filter(col("call_type").isin("voice", "sms", "data"))
...
def write_data_to_s3(
    df, output_path: str, format: str = "parquet", mode: str = "overwrite"
):
    ...
    try:
        if df is None:
            ...
            logger.info(
                f"Saving DataFrame to {output_path} in {format.upper()} format with mode={mode}..."
            )

            start_time = time.time()

            (df.coalesce(1).write.mode(mode).format(format).save(output_path))

            logger.info(
                f"Data successfully saved to {output_path} in {time.time() - start_time:.2f} seconds."
            )
...
def main():
    ...
    try:
        ...
        env_type, aws_access_key, aws_secret_key, aws_region, jar_path, base_path = (
            load_environment()
        )
