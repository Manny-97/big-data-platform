...
def generate_synthetic_sales_data(spark, n_records=100_000_000):
    ...
    df = (
        ...
        .withColumn(
            "Ship_Mode",
            expr(
                f"element_at(array({ship_modes_str}), int(rand() * {len(ship_modes)}) + 1)"
            ),
        )
        .withColumn(
            "Postal_Code",
            expr(
                f"element_at(array({postal_codes_str}), int(rand() * {len(postal_codes)}) + 1)"
            ),
        )
        .withColumn(
            "Region",
            expr(f"element_at(array({regions_str}), int(rand() * {len(regions)}) + 1)"),
        )
        .withColumn("Product_Reference", concat_ws("-", lit("PRD"), col("row_id")))
        .withColumn(
            "Category",
            expr(
                f"element_at(array({categories_str}), int(rand() * {len(categories)}) + 1)"
            ),
        )
        .withColumn(
            "Sub_Category",
            expr(
                f"element_at(array({sub_categories_str}), int(rand() * {len(sub_categories)}) + 1)"
            ),
        )
...
def compute_analytics(df):
    logger.info("Latest order date:")
    df.agg(max("Order_Date").alias("latest_order_date")).show()

    logger.info("\nTop Performing Categories:")
    df.groupBy("Category").agg(
        sum("Profit").alias("total_profit"), sum("Sales").alias("total_sales")
    ).orderBy(col("total_profit").desc()).show()

    logger.info("\nSales by Region and State:")
    df.groupBy("Region", "State").agg(
        sum("Sales").alias("total_sales"), sum("Profit").alias("total_profit")
    ).orderBy("Region", col("total_sales").desc()).show()

    logger.info("\n Average Shipping Delay by Ship Mode:")
    df.groupBy("Ship_Mode").avg("shipping_delay").orderBy("avg(shipping_delay)").show()
...
def main():
    ...
    logger.info(f"Total rows: {sales_df.count()}")
